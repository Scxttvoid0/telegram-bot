...
# APENAS EDUCATIVO O USO PARA UMA SITUAÇÂO REAL DESSE SCRIPT SERA BANIDO POS O SCRIPT È PARA FINS DE  ESTUDO ASS:Scxttvoid
...
# ============================================================================
# HANDLERS DE EDIÇÃO DE INTERFACE (CORREÇÃO PRINCIPAL)
# ============================================================================
async def editar_titulo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["editando_interface"] = "titulo"
    await query.edit_message_text("✏️ Envie o novo título:")

async def editar_subtitulo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["editando_interface"] = "subtitulo"
    await query.edit_message_text("✏️ Envie o novo subtítulo:")

async def editar_msg_dia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["editando_interface"] = "mensagem_dia"
    await query.edit_message_text("✏️ Envie a nova mensagem do dia:")

async def editar_avisos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["editando_interface"] = "aviso_novo"
    await query.edit_message_text("✏️ Envie um novo aviso para adicionar:")

async def editar_dicas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["editando_interface"] = "dica_nova"
    await query.edit_message_text("✏️ Envie uma nova dica para adicionar:")

async def salvar_interface(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    salvar_dados()
    await query.edit_message_text("✅ Interface salva com sucesso!", parse_mode="HTML")

# ============================================================================
# HANDLERS DE GERENCIAMENTO DE ADMs (CORREÇÃO PRINCIPAL)
# ============================================================================
async def editar_adm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handler para botão ✏️ editar ADM"""
    query = update.callback_query
    await query.answer()
    try:
        adm_id = int(query.data.replace("editar_adm_", ""))
    except:
        await query.edit_message_text("❌ ID de ADM inválido.")
        return
    
    if adm_id not in adms:
        await query.edit_message_text("❌ ADM não encontrado.")
        return
    
    context.user_data["editando_adm_id"] = adm_id
    msg = (
        f"✏️ <b>EDITAR ADM</b>\n"
        f"ID: {adm_id}\n"
        f"Nome atual: {adms[adm_id]['nome']}\n\n"
        "Envie o novo nome do ADM:"
    )
...
async def remover_adm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handler para botão 🚫 remover ADM"""
    query = update.callback_query
    await query.answer()
    try:
        adm_id = int(query.data.replace("remover_adm_", ""))
    except:
        await query.edit_message_text("❌ ID de ADM inválido.")
        return
    
    if adm_id == DONO_ID:
        await query.edit_message_text("🚫 Não é possível remover o dono!")
        return
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ Sim, remover", callback_data=f"confirmar_remover_{adm_id}"),
            InlineKeyboardButton("❌ Não", callback_data="gerenciar_adms")
        ]
    ]
    await query.edit_message_text(
        f"⚠️ Confirmar remoção do ADM {adms[adm_id]['nome']} (ID: {adm_id})?",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def confirmar_remover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirmação final da remoção"""
    query = update.callback_query
    await query.answer()
    try:
        adm_id = int(query.data.replace("confirmar_remover_", ""))
    except:
        await query.edit_message_text("❌ ID inválido.")
        return
    
    if adm_id in adms and adm_id != DONO_ID:
        nome = adms[adm_id]["nome"]
        del adms[adm_id]
        salvar_dados()
        await query.edit_message_text(f"✅ ADM {nome} (ID: {adm_id}) removido com sucesso!")
    else:
        await query.edit_message_text("❌ Operação inválida.")

# ============================================================================
# OUTROS HANDLERS (RESUMIDOS PARA ECONOMIZAR ESPAÇO - MANTIDOS IGUAIS AO ORIGINAL)
# ============================================================================
# ... [TODOS OS HANDLERS EXISTENTES MANTIDOS: solicitar_afiliado, menu_afiliados, converter_saldo_afiliado,
# vender_afiliados_validos, confirmar_venda_afiliados, painel_adm, ver_requisicoes, aceitar_requisicao,
# recusar_requisicao, cobrar_cmd, pago_cmd, controle_total, gerenciar_adms, adicionar_adm, editar_interface,
# estatisticas_menu, mostrar_estatisticas, alterar_periodo, definir_periodo, ban_cmd, unban_cmd, addsaldo_cmd,
# addprodutos_cmd, aviso_cmd, comprar_inicio, bin_escolhida, confirmar_compra, menu_perfil, recarga_info,
# solicitar_recarga, receber_valor_recarga, voltar_menu] ...
# (Mantenho todos os handlers originais exatamente como estavam no seu script)
...
async def confirmar_venda_afiliados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"Você será notificado quando processado.",
        parse_mode="HTML"
    )
...
# ============================================================================
# INICIALIZAÇÃO - ORDEM CORRETA DOS HANDLERS (CRÍTICO!)
# ============================================================================
def main():
    carregar_dados()
    ...
    # ============================================================================
    # CALLBACK HANDLERS - ORDEM CORRETA (ESPECÍFICOS → GENÉRICOS)
    # ============================================================================
    
    # 1. Handlers SUPER específicos (IDs numéricos de ADM)
    application.add_handler(CallbackQueryHandler(editar_adm, pattern=r"^editar_adm_[0-9]+$"))
    application.add_handler(CallbackQueryHandler(remover_adm, pattern=r"^remover_adm_[0-9]+$"))
    application.add_handler(CallbackQueryHandler(confirmar_remover, pattern=r"^confirmar_remover_[0-9]+$"))
    
    # 2. Handlers de edição de interface
    application.add_handler(CallbackQueryHandler(editar_titulo, pattern="^editar_titulo$"))
    application.add_handler(CallbackQueryHandler(editar_subtitulo, pattern="^editar_subtitulo$"))
    application.add_handler(CallbackQueryHandler(editar_msg_dia, pattern="^editar_msg_dia$"))
    application.add_handler(CallbackQueryHandler(editar_avisos, pattern="^editar_avisos$"))
    application.add_handler(CallbackQueryHandler(editar_dicas, pattern="^editar_dicas$"))
    application.add_handler(CallbackQueryHandler(salvar_interface, pattern="^salvar_interface$"))
    
    # 3. Handlers de compra (BINs específicas de 6 dígitos)
    application.add_handler(CallbackQueryHandler(bin_escolhida, pattern=r"^bin_[0-9]{6}$"))
    application.add_handler(CallbackQueryHandler(confirmar_compra, pattern=r"^confirm_[0-9]{6}$"))
    
    # 4. Handlers de afiliados/vendas
    application.add_handler(CallbackQueryHandler(confirmar_venda_afiliados, pattern=r"^confirmar_venda_afiliados_[0-9]+$"))
    
    # 5. Handlers de requisições (IDs com prefixo REQ_)
    application.add_handler(CallbackQueryHandler(aceitar_requisicao, pattern=r"^aceitar_req_REQ_[0-9]+$"))
    application.add_handler(CallbackQueryHandler(recusar_requisicao, pattern=r"^recusar_req_REQ_[0-9]+$"))
    
    # 6. Handlers de período (estatísticas)
    application.add_handler(CallbackQueryHandler(definir_periodo, pattern=r"^periodo_(hoje|semana|mes|todo)$"))
    
    # 7. Handlers de comandos ADM (texto)
    ...
    application.add_handler(CommandHandler("aviso", aviso_cmd))
    
    # 8. Handlers GENÉRICOS (últimos - capturam padrões amplos)
    ...
    # Handlers de texto (ordem de groups importantes)
